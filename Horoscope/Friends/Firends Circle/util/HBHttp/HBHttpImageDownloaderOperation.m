...
-(void)start
{
    ...
    if(!_request){
        if(_completeBlock){
            _completeBlock(nil,nil,[NSError errorWithDomain:NSURLErrorDomain code:HBHttpConnectionError userInfo:@{NSLocalizedDescriptionKey:@"请求创建失败！"}],NO);
        }
        return;
    }
    _executed=YES;
    ...
    if(connection){
        ...
        /****发送开始下载图片的通知***/
        [[NSNotificationCenter defaultCenter] postNotificationName:HBHttpImageDownloadStartNotification object:_url];
    }
    else{
        if(_completeBlock)
            /*****连接创建失败，返回错误信息******/
            _completeBlock(nil,nil,[NSError errorWithDomain:NSURLErrorDomain code:HBHttpConnectionError userInfo:@{NSLocalizedDescriptionKey:@"连接创建失败！"}],NO);
        return;
    }
     CFRunLoopRun();
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    if(_completeBlock)
        _completeBlock(nil,nil,[NSError errorWithDomain:NSURLErrorDomain code:HBHttpDownloadError userInfo:@{NSLocalizedDescriptionKey:@"图片下载失败！"}],NO);
    [[NSNotificationCenter defaultCenter] postNotificationName:HBHttpImageDownloadStopNotification object:_url];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response{
    ...
    CFRunLoopStop(CFRunLoopGetCurrent());
    if(_completeBlock){
        _completeBlock(nil,nil,[NSError errorWithDomain:NSURLErrorDomain code:[((NSHTTPURLResponse*)response) statusCode] userInfo:@{NSLocalizedDescriptionKey: @"图片内容为空！"}],NO);
    }
    [[NSNotificationCenter defaultCenter] postNotificationName:HBHttpImageDownloadStopNotification object:_url];
    [self done];
